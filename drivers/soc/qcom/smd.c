...
struct qcom_smd {
	...
	struct qcom_smd_alloc_entry *allocation_table;
...
void qcom_smd_resume(struct qcom_smd_channel *channel)
{

}
EXPORT_SYMBOL(qcom_smd_resume);
							...
							char *name)
{
	...
	ret = qcom_smem_get(SMEM_SMD_INFO_BASE_ID + channel_idx, (void **)&info, &info_size);
	if (ret)
	...
	ret = qcom_smem_get(SMEM_SMD_FIFO_BASE_ID + channel_idx, &fifo_base, &fifo_size);
	if (ret)
		goto put_smem_info;
...
put_smem_info:
	qcom_smem_put(SMEM_SMD_INFO_BASE_ID + channel_idx);
...
static irqreturn_t qcom_smd_edge_intr(int irq, void *data)
{
	 ...
	 * since last scan.
	 */
	available = qcom_smem_get_free_space();
...
static int qcom_smd_probe(struct platform_device *pdev)
{
	...
	ret = qcom_smem_get(SMEM_CHANNEL_ALLOC_TBL,
			    (void **)&smd->allocation_table, NULL);
	if (ret < 0) {
		if (ret != -EPROBE_DEFER)
			dev_err(smd->dev,
				"smd allocation table not available\n");
		return ret;
	};
