...
static struct max1187x_pdata *max1187x_get_platdata_dt(struct device *dev)
{
	...
	/* Parse gpio_pwr_en */
	pdata->gpio_pwr_en = of_get_named_gpio(devnode, "gpio_pwr_en", 0);
...
static int probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	input_mt_init_slots(ts->input_dev, MXM_TOUCH_COUNT_MAX, 0);
	ts->list_finger_ids = 0;
	...
	input_mt_init_slots(ts->input_pen, MXM_TOUCH_COUNT_MAX, 0);
	ts->list_finger_ids = 0;
...
static void max1187x_exit(void)
{
