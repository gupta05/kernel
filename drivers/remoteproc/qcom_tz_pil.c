...
static irqreturn_t qproc_fatal_interrupt(int irq, void *dev)
{
	...
	ret = qcom_smem_get(qproc->smem, qproc->crash_reason_id, (void**)&msg, &len);
	if (!ret && len > 0 && msg[0])
...
static int qproc_probe(struct platform_device *pdev)
{
	...
	qproc->smem = of_get_qcom_smem(pdev->dev.of_node);
	if (IS_ERR(qproc->smem)) {
		if (PTR_ERR(qproc->smem) != -EPROBE_DEFER)
			dev_err(&pdev->dev, "failed to acquire smem handle\n");
		return PTR_ERR(qproc->smem);
	}

	qproc->smd_edge_node = of_parse_phandle(pdev->dev.of_node, "qcom,smd-edges", 0);

	ret = of_property_read_u32(pdev->dev.of_node, "qcom,crash-reason", &qproc->crash_reason_id);
	if (ret < 0) {
		dev_err(&pdev->dev, "unable to parse qcom,crash-reason property\n");
		return ret;
	}
